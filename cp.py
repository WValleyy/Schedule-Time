        ...
        for (c,sub) in cla_sub:
                    ...
                    if pe == 6:
                        ...
                        model.Add(sum(Schedule[(t, c, sub, p_, s)]
                            for p_ in range(1,Period+1)) == pe).OnlyEnforceIf(teach)
                        model.Add(sum(Schedule[(t, c, sub, p_, s)]
                            for p_ in range(1,1+pe)) == 0).OnlyEnforceIf(teach.Not())
                    elif pe == 5:
                        teach =model.NewBoolVar('t')
                        tea = model.NewBoolVar('t')
                        model.Add(sum(Schedule[(t, c, sub, p_, s)]
                            for p_ in range(1,Period)) == pe).OnlyEnforceIf(tea).OnlyEnforceIf(teach)

                        model.Add(sum(Schedule[(t, c, sub, p_, s)]
                            for p_ in range(2,Period+1)) == pe).OnlyEnforceIf(tea.Not()).OnlyEnforceIf(teach) 
                        
                        model.Add(sum(Schedule[(t, c, sub, p_, s)]
                            for p_ in range(1,Period+1)) == 0).OnlyEnforceIf(teach.Not())
                    else: 
                        model.Add(sum(Schedule[(t, c, sub, p_, s)]
                        ...
                        for p in range(2,Period+2-pe):  
                            model.Add(sum(Schedule[(t, c, sub, p_, s)] 
                            for p_ in range(p,p+pe)) == pe).OnlyEnforceIf(Schedule[(t, c, sub, p, s)]).OnlyEnforceIf(Schedule[(t, c, sub, p-1, s)].Not())
                            
                        for p in range(Period+2-pe,Period):
                            model.Add(sum(Schedule[(t, c, sub, p_, s)] 
                            for p_ in range(p+1-pe,p+1)) == pe).OnlyEnforceIf(Schedule[(t, c, sub, p, s)]).OnlyEnforceIf(Schedule[(t, c, sub, p+1, s)].Not())
